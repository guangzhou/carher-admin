...
# **09-20 23:5x 再添三条**：200/201/202（入池前各自用独占直名 `cr-g-5.6-mini-<lane>`
# 实打过 200 + 暗号命中，SpendLogs 落点各归各位 —— 目录门禁 PASS 不算，195 就是
# 21 个 slug 全齐却恒 403）。
# **09-21 00:0x 再添三条**：204/206/207（同样先独占直名实打过）。203/205/208 那批
# 见 docs/cr-g-pool-swap-20260920.md 四节：203 是 still-anon（无 mailcom-fail），
# 205 是随机形状却 mailcom-fail —— 都不是 A 类，成因还没查。
# **09-21 01:2x 加 195**：它先前被判"21 个 slug 全齐却恒 403 的坏腿"，这轮平反 ——
# 绕开 litellm 直打 lane（`Bearer vscode` + tools）是 200 + 暗号命中，而 `/model/info` 里
# 195 **一行模型都没有**。不是腿坏，是**从来没登记**。已补 15 行并用独占直名实打过。
# ⛔ 82 不在池里：这天它也被直打判死（`session_expired`），且它的 28 个 `-82`/`-135`
# 独占直名已摘 —— 但 82/84 两条 Deployment 仍在跑，因为 `cursor-g-*` /
# `cursor-web-fc-*` 那两族对外名还挂在它们身上（待裁决，见 docs 七节）。
# **09-21 02:0x 加 196/197/198/199**（A 类重抓第一批成果，池腿 22→26）。
# 这四条走的是新脚本 lane_recapture_sheet_pw.sh：`.creds` 里的 mail_pw 是轮换前旧值，
# 密码改从飞书表取（逐号 sha256 比值，四条全是「盘 ≠ 表」）⇒ 抓号当场就过。
# 入池前四道判据齐了：seed 判据①②③ + in-pod `startedAt` 晚于 seed mtime；
# **绕开 litellm 直打 lane 200 + 命中暗号**（同轮 193 做阳性对照）；目录 21 slug 与 193 差集 0；
# 注册 60/60 + rollout restart 后用独占直名 `cr-g-5.6-mini-<lane>` 各实打 1/1 绿。
# **09-21 02:1x 再加 205**（池腿 26→27）。⛔ **同批抓成的 165/166 没进池**：它们直打 lane
# 是 200 + 命中暗号（腿完全活着），但 `/backend-api/models` 只回 **13 个免费档 slug**
# （有 `auto`、无 thinking/pro/instant，缺 ref 193 的 10 个），两次读一致 ⇒ 是账号档位，
# 不是刚登录的瞬态。这正是目录门禁存在的理由：**腿活 ≠ 该号买了订阅**。
# 两条 deploy 已 scale 0（YAML 备份在 198:/tmp/bk-zero-cursor-bpi-16{5,6}-20260921.yaml），
# seed 保留，等账号有订阅了 `scale --replicas=1` 再走注册。
# **09-21 02:5x 加 167**（27→28）。⛔ **168 也是免费档**（13 slug、有 `auto`、缺同样那 10 个），
# 与 165/166 同一处置：scale 0 + 留 seed，YAML 在 198:/tmp/bk-zero-cursor-bpi-168-20260921.yaml。
# ⇒ A 类重抓的号里已有 **3 个（165/166/168）是腿活但号没订阅**，它们不算抓号失败。
# **09-21 03:0x 加 172**（28→29），A 类重抓最后一个号，收尾：抓号 **11/11**，可入池 **7/11**
# （172/196/197/198/199/205/167），**4 个免费档**（165/166/168/169）scale 0 留 seed。
# 172 这一轮四道判据齐：seed ①②③ + in-pod `startedAt=2026-09-20T18:30:15Z` 晚于 seed mtime；
# 直打三极同轮（193 绿 / 172 绿 / **82 红 `session_expired`** —— 阴性对照也在同一轮，
# 只有正对照的"全绿"证不了尺子能红）；目录 21 slug 与 193 **双向差集 0**；
# 注册 15/15 + rollout restart + 独占直名 `cr-g-5.6-mini-172` 实打命中暗号。
# ⚠️ 尺子坑：/tmp/lanehealth.py 吃的是**逗号串**（`sys.argv[1].split(",")`），
# 写成位置参数会静默只打第一条腿 —— 我先踩了一次，当时那行"172 绿"其实没有对照。
POOL_LANES = ["167", "172", "175", "176", "177", "178", "180", "181", "182",
              "185", "186", "187", "188", "190", "191", "193", "195", "196",
              "197", "198", "199", "200", "201", "202", "204", "205", "206",
              "207", "208"]
# ⚠️ 「82 是 canary」这句已作废：**09-21 用户拍板摘掉 82/84**（两条腿 lane 级实打
# `session_expired`）—— 26 行模型行按 id 逐个删除（含两个曾被保护的裸载体
# `gpt-5.6-luna-wm`/`gpt-5.6-thinking`，删前已证各自只剩 1 行且都长在 82 上），
# 两个 Deployment `scale 0`。总行数 1928→1902 差正好 26。
# 备份：行 `198:/tmp/bk-crg-8284-rows-20260921.json`（只取证，密文不能回放）、
# YAML `198:/tmp/bk-zero-cursor-bpi-8{2,4}-20260921-scaledown.yaml`。
# 🔴 放行依据不是我先前说的「30 天零真实流量」—— 那句不准确，**有 3 发出自用户本人
# 的 key `cursor-liuguoxian04-5rub`**。真依据是更强的一条：这 20 个名字 **90 天共 17 发、
# 全在 09-20 一天、成功行 0**。判「有没有人在用」必须按 key alias 分组看时间跨度，
# 不能只看行数为 0 —— `failure` 行照样带 `prompt_tokens`。
# ⛔ 仍未动：`cursor-web-fc-terra{,-high,-max}` 3 个名字指向 0 副本的 base 服务
# （id 是 `zerokey-cursor-web-fc-101-*`，废弃的 lane 101 线），以及 `zero-cursor-101`。
# 195 入过池又摘了（09-20 22:2x）：账号侧恒 403（`[ChatGPT] Got 403`，3/3），
# 同轮 193 是绿的 ⇒ 不是尺子坏。留一条恒 403 的腿在池里 = 1/15 的流量被打黑，
# 比少一条腿糟得多。摘腿备份 /Data/backups/crg-retire-lane195-20260920-222440.txt。
POOL_NAMES = [
