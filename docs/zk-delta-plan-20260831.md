...
#### S8-4 把 ⑨ 从「还差一步」改成一条命令

写 S8 的时候翻出一处**我自己写的假话**：README 和本文都写着「切过去后设 `ZKD_CAPTURE`
就能采，回灌重跑即转绿」，但 `switch.sh` 装 LaunchAgent 时**压根没把 `ZKD_CAPTURE` 传进去**。
也就是说切过去之后什么都不会攒。⑨ 之所以一直红，「切过去」和「验完」之间那一步手工设环境变量
一直没人做——而那一步的存在本身是我漏掉的，不是谁偷懒。

改：`switch.sh on` 默认开采集（`CAPTURE_MAX=0` 可关），落到 `tests/fixtures/`。

**封顶不是可选项**：真实 body 每条几 MB、Cursor 每轮一发，不封顶跑一天能把本机磁盘写爆，
而金样几十条就够用。加 `ZKD_CAPTURE_MAX`（默认 40）+ `ZKD_CAPTURE_MAX_MB`（默认 512），
采够自己停手并打 `capture_done`，进度露在 `/metrics.json`。

**实测而不是断言**：上限设 3、发 5 发 → 恰好落 3 个文件、内容逐字节相同、日志 `why=file_cap`；
把这 3 条当 fixtures 回灌 → ⑨ 转绿、**45 绿 0 红**；删掉再跑 → 回到 44 绿 1 红。
「回灌即转绿」现在是验过的，不是写着的。

`tests/fixtures/*.json` 进 `.gitignore`——**采到的是真实会话正文和代码，绝不入库。**
...
### S9 · 回归计划自跑（08-31，事实标 [实测]）

按「先写回归计划，再自己全跑」的要求，先落了 `docs/zk-delta-regression-20260831.md`（R0–R8），
再逐项执行。**R0–R7 全部通过**，完整数据在那份文档里。这里只记这一轮翻出来的**真问题**——
顺风的数据留在回归文档，教训留在这里。

#### S9-1 `switch.sh on` 从来没真的切过地址

报错 `找不到 Cursor 资源目录: /opt/homebrew/Cellar/node/25.9.0_2/Resources/app`。两个 bug 叠着：

- 脚本用**系统 node** 跑 `cursor_team_setup.js`，而那个安装器靠 `process.execPath` 推导 Cursor
  安装目录，必须用 Cursor 自带的 Electron 以 node 模式跑。用系统 node 跑，它就去 Homebrew 的
  node 前缀底下找 Cursor。
- 调用没带 `--apply`，就算路径对了也只是 dry-run，什么都不写。

**所以 S8 里那句「⑨ 只差一条命令」是假的**——那条命令按下去只会报错。
这是继 S8-4「`ZKD_CAPTURE` 没传进去」之后，同一个位置的第二处「写了没跑过」。

#### S9-2 更要命的：不能拿安装器当开关

`cursor_team_setup.js` 的 `mergeConfig()`（473-477 行）：选中模型名不以 `cursor-g` 开头时，
把 composer / cmd-k 的 `modelName` **覆盖成 `cursor-g-5.6-sol`**。
查本机 state.vscdb，当前 composer 正是基准 **`cursor-web-fc-82-terra`**。

也就是说，如果按原计划只补一个 `--apply` 就跑过去，会把门①门② 的基准模型**静默换掉**，
后面所有对照全部作废，而且不报任何错。这不是"顺手多做了点事"，这是把量具本身换了。

修法：新写 `zk-delta/cursor_baseurl.js`，只动 `openAIBaseUrl` 一个字段，写完当场读回来验一条
不变量——把地址换回旧值之后两个 blob 必须完全相同，不成立就自动还原并退非零。
证伪腿 [实测]：故意让它多改一个字段 → sha `2a79a1a3 → 5bf9eb90`（先确认真改到了行，
否则绿灯是假绿）→ 退出码 5、自动还原、零残留 → 还原代码 → 恢复正常。

**切地址和装机是两件事，不该共用一个入口。** 复用省下的那点代码，代价是量具被换掉。

#### S9-3 采集差点拿我自己的输出给自己发绿灯

第一次采完去看 `fixtures/`，3 条全是 93 B 的 `"hi"`——那是 `switch.sh on` 里我自己那条自检 curl，
每 `on` 一次落一条。**拿这些让 ⑨ 转绿，就是拿我自己造的样本给自己发绿灯。**
同时 Cursor 启动时那发 `GET /v1/models` 也被采成 0 字节文件，⑨ 一 `JSON.parse` 就崩。

修法：采集加两道门——UA 必须是 Cursor（挡自检 curl，记 `capture_skipped_ua`）、
且必须是有正文的聊天请求（挡 0 字节）。污染文件已删。

**证据的来源必须和被验对象无关，这条要在采集层挡住，不能等到判读时再想起来。**

#### S9-4 ⑨ 真绿了，门① 也升级到真 GUI

修完之后 [实测]：GUI 驱动 8 发全部 `fired_ok`，采到 **22 条真 Cursor body**（115–139 KB/条），
`capture_skipped_ua: 0`，无 0 字节文件；`run.js` → **45 绿 0 红**，含
`✓ ⑨ 真实抓包每一发上游收到的都逐字节相同（n=22）`。

门① 也顺势从"我构造的 `ls` 请求"升级成 `tests/live_gate1.js`：取 fixtures 里最小的一条真抓包，
等长替换标记后两腿各打一次，再去 `deploy/zero-cursor-bpi-82` 日志比它两次**实际收到**的
`[PROMPT] REQ` 块——**A 腿 148 行、B 腿 148 行、归一化后零差异**，两腿都是
`promptLength: 11945  attachments: 0`，无转录体行标。

比"重建字节相同"值钱的地方：后者只证到 LiteLLM 入口，前者一路证到网关，
覆盖中间任何一跳可能做的手脚。

#### S9-5 巡检自己有毛病：把 401 和 400 炖成一个数

`audit.sh` 第 6 组报红「上游非 2xx 1 发」。按纪律先查数据：服务端日志里 `up_out=93` 的 turn
有 5 发（自检 `"hi"` 正好 93 B）。证伪腿 [实测]：再发一发故意不带 Key 的自检 →
`req 76→77`、`upstream_non2xx 1→2`、`upstream_2xx 73 不变`，一发不多一发不少，坐实了。

**不是缺陷，但规则是缺陷**：401 是客户端凭据问题，我们只是忠实透传；400 才是要盯死的那个形状。
见 401 就报漂移的巡检，喊几次狼就没人看了。

改：服务端加 `upstream_by_status` 分状态码计数，巡检把 401/403 归为不计入异常。
两个方向都验 [实测]：打 401 → 巡检绿；打 400（不存在的模型）→ 巡检红
`上游硬失败 {"400": 1}`。规则不是摆设。

顺带修 `apply.sh` 一个真 bug：它在 198 上跑 `node --check`，而 **198 根本没装 node**，
这一步一直是直接失败退出的。改成本地做语法检查、传过去比 **sha256**——后者才是这一跳真正要证的事。

上线复验 [实测]：指纹 `d7ffba2adff793e1 → 62c8abc43a665383`，rollout 成功，
巡检**六组全绿**，真链路 5 轮冒烟 VERDICT = PASS。

#### 这一轮的一句话

三处问题形状完全一样：**写了，没按下去过。**
`ZKD_CAPTURE` 没传进去、`switch.sh on` 跑不通、`node --check` 在没装 node 的机器上跑——
都是「代码看起来对」和「这条路真的走通过」之间的差。
以后凡是写进文档的操作步骤，交付前必须至少真跑一次，跑不通的不许写成"只差一步"。

---

### S10 上正式环境：并进装机安装器全员发（08-31）

用户口径两句：**「没问题，那就上正式环境。」** + **「不用影响 codex 7d 那条线。」**
前一句是把 zk-delta 从"只在我本机"变成全员发；后一句是一条硬约束。

commit `180d947`。

#### S10-1 分发路径：小代理必须能在没装 Node 的机器上起来

分发包 `cursor-g-setup.zip` 的整个卖点是**"不用装 Python / Node"**。而 zk-delta 的小代理是
Node 程序。这里有个会直接把同事的 Cursor 弄坏的坑：

launchd plist 里如果写 `node`，同事机器上没有它 → 服务起不来 → 而这时安装器**已经**把
Cursor 的 `openAIBaseUrl` 指到了 `127.0.0.1:8788` → **Cursor 直接连不上任何模型**。

修法两条，都是刻意的：

1. **用 Cursor 自带的 Electron 跑**（`ELECTRON_RUN_AS_NODE=1` + Cursor 二进制），
   这正是安装器本来就在用的招。[实测] 先在一个备用端口上真跑起来，确认可行，才敢在 plist 里依赖它。
2. **顺序：先起小代理，healthz 通了才改地址。** 装 zk-delta 那一步放在 `mergeConfig()` **之前**，
   12 秒内 healthz 没响应就打日志、保持公网直连、并明说"本次不改 BYOK 地址"。
   顺序倒过来就是上面那个坏法。

#### S10-2 同事那边抓包必须硬关

小代理默认把请求体落到 `tests/fixtures/` 攒金样——那是**给我自己**验证 ⑨ 用的。
在同事机器上开着，就是**把别人的真实工作内容写到他自己磁盘上**。分发路径里硬写 `ZKD_CAPTURE_MAX=0`。

#### S10-3 安装器不是开关：`--keep-model` / `--zk-delta-only`

S9-2 记过：`mergeConfig()` 在选中模型名不以 `cursor-g` 开头时会把 composer 覆盖成
`cursor-g-5.6-sol`，而本机基准正是 `cursor-web-fc-82-terra`。这轮既然要复用安装器做分发，
就必须把这条堵上：新增 `--keep-model`（`--zk-delta-only` 隐含它），带上之后 `mergeConfig`
跳过模型覆盖并打印 `(skip, --keep-model：保持 cursor-web-fc-82-terra 不动)`。

**[实测] 装 / 卸 / 再装三趟，composer 全程 `cursor-web-fc-82-terra` 没动过。**
并且验的是**从解出来的 zip**，不是源码树——同事拿到的是前者。

`--revert` 也补了 `zkdUninstall()`：回滚把地址还原成公网，留着服务就是个占着 8788 的孤儿。

**诚实边界**：只在 macOS 实测过。Windows/Linux 没有 launchd，这一步直接跳过并说明，
不写一个没验证过的服务定义假装支持。

#### S10-4 打包脚本两个潜伏 bug

- 中文文件名在 zip 里跨平台乱码：`关掉增量传输-Mac.command` → `unzip -l` 显示 `?��??`。改 ASCII。
- **`zip -r` 是往已有归档里追加，不是重建**：改完名之后旧的乱码条目**仍然留在包里**。
  这是打包脚本从第一天就有的 bug，只是这次是第一次改文件名才暴露。改成先 `rm -f` 再打。

#### S10-5 容量：这台单副本到底能扛多少人

上线给全员之前必须回答一个之前没量过的问题：**同时多少人挂着会话，这个单副本会 OOM？**

写了 `zk-delta/tests/capacity_probe.js`（纯本地起服务端灌数据，**一发不打上游**），
灌到 277MB store。这一段我连着判错两次，两次都是**判据本身**的毛病：

| | 错在哪 | 后果 |
|---|---|---|
| 第一版 | 用 `rss/store` 这个**总倍率**反推上限 | 总倍率被固定基座污染（1 条会话时实测 **20.17x**），算出"生产 800MB 必须调小到 1204MB"这种荒唐结论 |
| 第二版 | 把**探针自己的** `ZKD_MAX_BYTES` 读回来当被判对象 | 那个值是我为了做实验故意放大到 64GB 的 —— **在量自己的旋钮** |

修正后的判法：拟合**边际倍率** `rss = base + k·store`，且生产的两个上限**从参数进来**、
探针自己那两个旋钮单独标注"已故意放大"。另外两点法斜率在 GC 噪声下不稳
（同一份代码连跑两次给出 1.01x 和 1.25x），改成最小二乘跑全部样本、再取相邻点最大斜率做保守值。
...
生产 ZKD_MAX_BYTES=800MB 撑满 → RSS ≈ 1169MB，2Gi limit 下安全 → 不用改
生产 ZKD_MAX_CONV=400 单独    → 1847MB store → RSS ≈ 2471MB → 拦不住，会 OOM
⇒ 真正在拦的是字节上限，两条都得留着

20 人 × 8 条活跃会话 =  739MB  在上限内
40 人 × 8 条活跃会话 = 1478MB  超上限 → 淘汰最久没用的（那条下一发退化成全量，不出错）
```

「每人 8 条活跃会话」是**假设不是实测**——我只有自己一台机器的行为数据。已在输出和文档里标明。

**我自己那个"必须调小"的结论被这轮推翻了**，留下来的是另一个真发现：条数上限单独拦不住。

#### S10-6 把静默 OOM 变成事故**之前**会红的巡检

为什么这条值得单开一件事做：单副本 + 会话状态在进程内存里 ⇒ OOMKill 一次是**所有人**的会话全丢，
**而且不报错**，只是每条会话下一发退化成全量。事后只能从 `restartCount` 反推，那时候已经丢过一轮了。

`server.js` 加 `memStats()`（`rss_bytes` / `store_bytes` / `store_max_bytes` / `conv_max` /
`conv_avg_bytes`），`audit.sh` 加第 [7] 组三条腿：眼下水位（70% 黄 / 85% 红）、
按边际斜率外推"把配的上限撑满会到多少"、条数上限单独够不够。
[实测] `limit 2048MB，当前 RSS 52MB（3%）… 撑满 → RSS ≈ 1044MB … ✓ 撑满也在 limit 内`，
**巡检 7/7 全绿**。

#### S10-7 「不影响 codex 7d」——我的尺子先是错的

写了 `zk-delta/k8s/codex7d_guard.sh`，先按 7 天滑动窗口做前后快照 `diff`。
结果 `diff` 报出一堆 `!` 和「codex 侧有新增请求」。

按纪律不能就此收工，也不能把它解释掉，得拿数据看是谁的流量：

```
03:00 那一小时   gpt-5.6-sol              4165 发   ← 全公司在正常上班
                local-deepseek-v4-flash    26 发   ← 我自己（14 + 12）
我开工前两小时   gpt-5.6-sol         4651 / 2869 发
```

**差值全是别人的正常业务。7 天窗口是滑动的，前后差只在集群空闲时才等于"我打的量"；
上班时段拿这个 diff 判"我有没有碰 codex"是 100% 假阳。**
我的尺子在我要用它的那个时段是无效的。

有效的判据是**按模型归因**：我打了哪个模型是**我自己控制的**，别人的流量落在别的模型名上。
已把有效范围写进脚本头，并新增 `mine` 子命令（按 小时 × model_group）。

于是结论[有数据]：**整夜上游 26 发全部落在 `local-deepseek-v4-flash`（自建 GPU 盒），
ChatGPT 侧零消耗，acct-82 的 24h/7d 窗口没动。**

再加一条不依赖我的自律的结构性理由：**重建发生在 LiteLLM 之前**，上游收到的字节与不装
zk-delta 时逐字节相同（今晚 26/26 验过）⇒ zk-delta **按构造**就改不了 ChatGPT 侧的配额消耗。
能吃额度的从来只有我的回归流量，而那个已经全部导到自建盒了。

#### S10-8 上线前的回归（门在退出码上）

| | 结果 |
|---|---|
| 离线 `run.js` | **45 绿 0 红**（⑨ `n=23`），退出码 0 |
| 真流量多轮（打 `local-deepseek-v4-flash`，不烧 ChatGPT 额度）| **12/12 PASS**，省 **86.2%** |
| 字节对账 | 上行 1,646,888 == 集群 `bytes_in`；原始 13,652,229 == `bytes_out`，两侧**完全相等** |
| 从解出来的 zip 装 / 关 / 再装 | 全过；关掉后 8788 拒连、地址回 `cc.auto-link.com.cn/pro/v1`、composer 未动 |
| `audit.sh` | **7/7 绿** |

其中一发 **413** 出现在 14 轮那次。按三段式查而不是绕开：
假设"zk-delta 导致"→ 证伪条件"若成立，同一路径缩小 body 也该失败，且集群应记为自己的拒绝"→
数据：863KB 那轮同路径 **12/12 全绿**；集群记的是 `upstream_by_status {"413":1}` 且
`reject_409 = 0`、`rebuild_ok 14/14`；错误文本是 198 的脱敏包装而非 zk-delta 的 `zkd_body_too_large`。
**假设推翻**——那是 `local-deepseek-v4-flash` 上游自己 ~1.64MB 的 body 上限。

#### 这一轮的一句话

上一轮是「写了，没按下去过」。这一轮是**「量了，但尺子是错的」**——
总倍率量内存、滑动窗口量额度、探针量自己的旋钮，三次都会给出一个说得通但错的数。
**判据本身也要过证伪**：先问"如果这把尺子坏了，数据会长什么样"，再去看数据落在哪。
