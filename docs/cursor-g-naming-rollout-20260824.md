...
**压测记录(2026-08-24 22:2x–22:3x,用户点名场景)**:
- 连续多条消息(同会话 5 连发 ×2,含"继续"/"冒泡排序"短消息):**10/10 PASS**,增量续发全程
  命中(同 conv id,每轮只透传 ~100 字符),零泄漏。
- **不等回复就发下一条**(同会话并发 3 连发 ×2 + 风暴后追问):**8/8 PASS**;风暴后问"建了几个
  文件"答"3"(状态无丢失)。机制:竞态请求各自指纹 miss → 各开独立全量会话("增量是优化,
  全量是地板"),从不共享 conv id 故无上游同会话冲突;保险丝本轮又救回 2 条坏 JSON
