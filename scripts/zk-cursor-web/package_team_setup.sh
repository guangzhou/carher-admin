...
━━━ 已经装过的同事:双击 UPGRADE(不是 INSTALL,也不是 REPAIR)━━━
  ...
  3) 重打 Cursor 界面补丁(等于 REPAIR 那一步)。**本次(09-22)新增一条**:
     ...
     **本次(09-23)再新增一条**:修掉「发消息报 An unexpected error occurred /
     Request ID: xxxx」以及「This model does not support custom API keys」那两个报错。
     原因是 Cursor 最近给部分账号灰度开了一个"本地轮次"开关,开着的机器会把你的
     请求交给 **Cursor 自家的推理服务**,而那个服务不认我们的 Key(也不支持自定义 Key),
     于是必然报错 —— 跟你的 Key、网络、选哪个模型都无关,纯看你这台有没有被灰度到。
     补丁把这个开关对我们的模型关掉,请求回到原来那条能用的路上。
  3b) Windows 上光跑 REPAIR 不够,要跑 **UPGRADE**:REPAIR 按设计不碰配置/开关,
     而上面那个病有一半在"BYOK 开关被翻成了关",只有 UPGRADE/INSTALL 会把它写回来。
