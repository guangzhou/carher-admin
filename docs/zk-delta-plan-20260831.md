...
## S11 巡检红了，红在我自己的产物上（08-31 上午）

### S11-1 现象

全员发做完之后又跑了一遍只读巡检，它**红了**：

```
[6] 运行计数器
     请求 30（增量 27 / 全量 3），重建成功 30，409 0，上游非2xx 1
     上游非2xx 分布： {"413": 1}
  异常：上游硬失败 {"413": 1}（400 尤其要查）
结论：有漂移
```

就是 S10 里那一发 413——已经定因为 `local-deepseek-v4-flash` 上游自己
~1.64MB 的 body 上限，**与 zk-delta 无关**。

最省事的做法是把判据放松让它变绿。**没有这么做**：为了让自己的产物过关去改判据，
是 S10 那三次「尺子错了」的第四次，只不过前三次是不小心，这次会是故意。

### S11-2 真问题不在阈值，在计数器的形状

进程里的计数器是**累计**的，没有时间信息。于是「狼来过一次」和「狼正在咬人」
在巡检眼里长得一模一样。后果不是漏报，是**警报永远关不掉**——这一发 413 会把
巡检钉成红色直到有人重启 pod，而关不掉的警报等于没有警报。

同一个形状今晚已经修过一次（401/403 混在非 2xx 里，见 S10）。当时的话是
「喊几次狼之后就没人看了」。**这次是狼真的来过一次，然后警报再也关不掉。**

### S11-3 修法

| 位置 | 改动 |
|------|------|
| `server.js` | 补 `upstream_last_hard_at`（最后一发硬失败时刻）+ `upstream_ok_since_hard`（之后连续干净发数）。401/403 **不进**这两个字段，否则语义又被稀释 |
| `audit.sh` 第 [6] 组 | **400 一发就红、不参与降级**（专门在猎的形状）；其余硬失败要 **≥100 发连续干净且 ≥2 小时没再犯** 才降级成 `!`，降级后**照样每次打印**；老服务端没这两个字段则退回「分不清」并**保持红** |

### S11-4 验收

把第 [6] 组判定逻辑单独抽出来灌构造数据，六条分支全部落在预期：

| 构造场景 | 期望 | 实际 |
|----------|------|------|
| 全干净 | 绿 | exit 0 ✓ |
| 413 在 9 小时前，之后 250 发干净 | 降级为「留意」 | exit 3 ✓ |
| 413 刚发生 1 分钟，之后 3 发干净 | 红 | exit 1 ✓ |
| 含 400，99 小时前 + 之后 9999 发干净 | **仍然红** | exit 1 ✓ |
| 只有 401 | 不算异常 | exit 0 ✓ |
| 老服务端无新字段 | 保持红 | exit 1 ✓ |

推上去之后：离线 `run.js` **45 绿 0 红**（exit 0）；真流量 6 轮打
`local-deepseek-v4-flash`（不碰 ChatGPT 额度，不影响 codex 7d 那条线）
**6/6 PASS**、逐字节重建 6/6、省 75.7%；巡检 **7/7 绿**、exit 0。

### S11-5 诚实边界

**这次的绿有一半是重启换来的，不是新逻辑判出来的。**计数器活在进程内存里，
pod 一重启全部归零，那一发 413 是**直接消失**了，而不是被新逻辑判成「历史事件」。
真正证明新逻辑管用的是 S11-4 那张构造数据表。新逻辑真正要管的是**下一次**：
服务跑着跑着偶发一发硬失败，巡检不会因此永远红到有人去重启它。
这句话已经写进 `audit.sh` 的注释里，免得下次看到绿的人误会。

---

## S12 收口：把「靠我记得」变成「工具拦得住」（08-31 上午）

### S12-1 apply.sh 从来没跑过回归

写完 S11 又回头审部署链路，发现一个和 S11 同类的洞，只不过不在巡检里，在**部署脚本**里：
`apply.sh` 只有 `node --check`。语法检查只能证明"文件能被 parse"，证明不了
**重建出来的字节还对**——而后者是这个服务的**全部**价值。

**今晚每一次绿灯，都是我记得手动跑一遍 `run.js` 换来的。门在人的记性里，不在工具里。**

补法：步骤 0 跑 `tests/run.js`，退出码不为 0 就停；`ZKD_SKIP_TESTS=1` 救火开关会大声打印。

### S12-2 双向证伪（一道没拦下过东西的门只是一句注释）

| 腿 | 做法 | 结果 |
|---|---|---|
| 坏文件 | 往 `framing.js` 注入**行为性**错误（重建丢最后一条 item） | `node --check` **照样过**；`apply.sh` 退出码 **1**，日志里「传源码到 198」出现 **0 次**，没碰集群 |
| 好文件 | 还原后再跑 | 退出码 **0**，`PASS 45 FAIL 0`，指纹 `a6d0e89d7ffe6217` 未变（没触发重启） |

### S12-3 顺手修掉两个 /tmp 固定路径

`/tmp/zkd-cm.yaml`、`/tmp/zkd-deploy.yaml` 生成失败时，路径上会躺着上一次（甚至别人）
的陈旧文件被推上生产，deploy.yaml 那条连非空检查都没有。改 `mktemp -d` + `trap`，
补"非空 + 占位符确实被替换掉"两道检查。

### S12-4 沉淀成 skill

新增 `.claude/skills/zk-delta-ops/`。与 `zk-cursor-web-fc-iterate` 分工：后者管**网关侧**
`responses.js` 的会话复用，前者管**客户端→LiteLLM 这一跳**的公网增量。收口的是**判据类**
坑而非操作步骤：累计计数器分不清历史与正在发生、重启后的绿不算验证、内存按边际不按总倍率、
codex 额度只能按模型归因、安装器默认路径会换掉量具、`zip -r` 是追加不是重建。

### S12-5 一句话

**S11 是判据坏了（警报关不掉），S12 是判据根本没接上（回归没人跑）。**
两处都不靠"以后小心点"解决，靠把判据焊进工具、并且**实测它真的会拦**解决。
