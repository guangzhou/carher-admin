#!/usr/bin/env python3
"""Add GPT-6 Astra to every scoped 198 her (``carher-*``) key.

The 2026-09-05 rollout put both ``gpt-6-astra`` and ``chatgpt-gpt-6-astra`` into
the her keys' ``models`` allowlist but never added the bare-name alias, so the
bare name was visible in ``/v1/models`` yet routed to a 400
``ProxyModelNotFoundError``. This script closes that gap the same way
``litellm-cursor-add-astra.py`` does for Cursor keys.

Additive only: existing models and aliases are preserved, never replaced. The
key API replaces whole ``models``/``aliases`` fields, so every update is
...
def main() -> int:
    ...
    print(f"her_keys={len(keys)} planned={len(planned)} unrestricted_skipped={unrestricted}")
    ...
    # The management API silently ignores unknown fields and still returns 200,
    # so a write is only proven by reading the row back.
