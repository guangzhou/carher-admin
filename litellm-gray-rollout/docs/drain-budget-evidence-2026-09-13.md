...
## 7. 执行当天重量（2026-09-14 01:20，同窗口 `zkreq.log` + `.1`）

§5 那句"执行当天必须重量一次（流量形状会变）"的落实。窗口 = 2 天，`/pro` 有效样本
**743,620**（比 09-13 的 1,243,452 少四成 —— 轮转点不同，不是流量腰斩）。

| uri_class | count | p50 | p95 | p99 | **p100** |
|---|---:|---:|---:|---:|---:|
| other | 689,550 | 0.01 | 1.05 | 1.97 | 600.15 |
| **responses** | 45,398 | 9.01 | 74.10 | 391.73 | **111,539.51** |
| chat | 7,371 | 8.82 | 34.09 | 101.85 | 904.58 |
| messages | 1,296 | 11.14 | 77.06 | 175.79 | 902.71 |
| image | 5 | 51.56 | 54.36 | 54.36 | 54.36 |
| **ALL /pro** | **743,620** | 0.02 | 6.33 | 27.99 | **111,539.51** |

| 阈值 | 超阈条数 | 占比 | 构成 |
|---:|---:|---:|---|
| 60 s | 3,536 | 0.4755% | responses=3223 chat=171 messages=136 other=6 |
| 300 s | 620 | 0.0834% | responses=579 chat=36 messages=4 other=1 |
| **570 s**（当前预算） | **406** | **0.0546%** | responses=376 chat=28 other=1 messages=1 |
| 900 s | 304 | 0.0409% | responses=302 chat=1 messages=1 |
| 1800 s | 240 | 0.0323% | responses=240 |
| 3600 s | 128 | 0.0172% | responses=128 |
| 7200 s | 65 | 0.0087% | responses=65 |

### 7.1 两处必须改写执行单的差异

| 项 | 09-13 | 09-14 | 判读 |
|---|---:|---:|---|
| p100 | 81,665 s（22.7 h） | **111,539 s（31.0 h）** | 结论方向不变且更强：抬 grace 覆盖 p100 要 31 小时，仍不可行 |
| >570 s 占比 | 0.0940% | **0.0546%** | 截断成本**下降**，仍走"显式接受截断" |
| 504 条数 | 28（0.0039%） | **28（0.0038%）**，28/28 落在 560~610 s，最长 600.15 s | §6 那张表原样成立 |

### 7.2 ⚠️ §3.2 那句"截断影响面明确且单一"**当天被证伪了**

09-13 的 UA 表结论是「清一色 Codex（Desktop + TUI），没有 CarHer / cursor / 其它 lane
混在里面」。09-14 的同一张表：

| 条数 | User-Agent（截断 40 字符） |
|---:|---|
| 60 | `Codex Desktop/0.154.0-alpha.6.2 (Windows` |
| 42 | `codex-tui/0.154.0 (Windows 10.0.22631; x` |
| 40 | `Codex Desktop/0.153.4 (Mac OS 26.2.0; ar` |
| 33 | `Codex Desktop/0.153.4 (Windows 10.0.2620` |
| 28 | `Codex Desktop/0.148.0-alpha.21 (Mac OS 1` |
| **27** | **`Cursor/3.18.25`** |
| 21 | `codex-tui/0.153.4 (Mac OS 26.5.1; arm64)` |
| 20 | `Codex Desktop/0.153.4 (Mac OS 26.4.0; ar` |

**Cursor 进了尾巴**（27 条 > 570 s）。"影响面单一"这句话在 09-13 是真的，今天不是。

这正是"当天重量"这条纪律的产出：一份**上一次量对了**的结论，
在不重量的情况下会原样进执行单，并把一个新的受影响消费者写没。
⇒ 执行单 §147 的"受影响消费者"必须写成 **Codex Desktop / codex-tui + Cursor 3.18.25**，
截断公告的收件人也要加上 Cursor 那条 lane。
