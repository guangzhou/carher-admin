...
function handsBlock(env) {
  ...
  return [
    ...
    // 能力发现：外部 CLI（飞书 lark-cli 等）就在用户机器上，用 ⟦cmd⟧ 就能使 ——
    // 2026-08-10 现场：用户要建飞书文档，模型明明摸对了 lark-cli 方向，却因
    // 不确定"自己能不能用"而空承诺拖延三轮。规则：不确定某 CLI 是否可用时，
    // 先探测（which/--help），在，就直接用它完成任务；绝不空口宣告"我现在去做"。
    '用户机器上的 CLI 工具（如飞书 lark-cli）都可通过 ⟦cmd¦run=…⟧ 真实调用；',
    '不确定是否可用时先 ⟦cmd¦run=which {tool}⟧ 探测，存在就直接用它完成任务。',
