...
| 用户向你报"前端 / 第三方 dashboard 消费明细查不出"，日志里看到 `Exception: Only proxy admin can be used to ... Route=/spend/logs/v2. Your role=unknown. Your user_id=xxx****xx` + `GET /spend/logs/v2?api_key=...&page_size=N HTTP/1.1 401` | 客户端用普通 `sk-...` user-key 调了 admin-only 端点 `/spend/logs/v2`；非 master / 非 `proxy_admin` 一律 401 | **首选**：让前端改打旧版 `/spend/logs?api_key=<token>&start_date=...&end_date=...`（owner 可见，已实测 200）。**次选**：在 carher-admin 后端 `backend/main.py` 加一个 `/api/litellm/spend/logs?key=...` 转发路由，由后端用 master key 代查并按 user 维度过滤。**最次**：把那把 key 升 `user_role=proxy_admin`（不推荐，权限太大） |
| 用户向你报 "LiteLLM Web UI 搜不到我的 key" / "前端搜 `claude-code-zhangsan` 返回空" | LiteLLM UI 的 "Search by Alias" 输入框走 `/key/list?key_alias=<exact>` **精确匹配**，不是 LIKE/子串匹配；老的 keygen 脚本给 alias 加了 4 字符随机后缀（如 `claude-code-zhangsan-50gj`） | 让用户搜完整带后缀的 alias；或用 CLI 一行查全名：`curl -s "http://127.0.0.1:4000/spend/keys?limit=600" -H "Authorization: Bearer $MK" \| jq '.[] \| select(.key_alias \| test("zhangsan";"i")) \| .key_alias'`；长期可用 `/key/update` 把 alias 改成不带后缀的 |
...
### ⚠️ TZ 陷阱（**写 SpendLogs SQL 前必读**）

`LiteLLM_SpendLogs.startTime` / `endTime` 列类型是 **`timestamp without time zone`（naive）**，
LiteLLM 写库时塞的是 **真实 UTC 值**，但列本身不带 TZ 元信息。
litellm-db pod 的 session TZ = `UTC`（实测 `current_setting('TimeZone')` = `UTC`，`now()` = 真实 UTC）。

PG 的 `AT TIME ZONE` 在两种列上**方向相反**，是历史踩坑高发区：

| 列类型 | `expr AT TIME ZONE 'X'` 的语义 |
|---|---|
| `timestamp without time zone`（naive） | "**把这个值当作 X 时区的本地时刻** → 转成 TIMESTAMPTZ（绝对时刻）" |
| `timestamp with time zone`（TIMESTAMPTZ） | "**把这个绝对时刻在 X 时区的本地墙钟显示** → 转成 naive timestamp" |

所以对 naive 列 `startTime`（值=15:01 UTC）写 `startTime AT TIME ZONE 'Asia/Shanghai'`，
PG 会理解成 "把 15:01 当成 BJT，所以这是 15:01 BJT = 07:01 UTC 这个绝对时刻"
→ to_char 在 session UTC 里显示 → "07:01"（**比真实 BJT 23:01 少了 8h，方向相反**）。

#### 标准模板：UTC 与 BJT 双列展示

```sql
-- naive UTC 列 → 既要 UTC 显示也要 BJT 显示
to_char(sl."startTime",                                                   'MM-DD HH24:MI')       AS utc,
to_char((sl."startTime" AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Shanghai', 'MM-DD HH24:MI')       AS bjt
```

要点：
1. `utc` 列直接 `to_char` 原列即可（值本就是 UTC）；
2. `bjt` 列**必须双层 AT TIME ZONE**：先 `'UTC'` 把 naive 升成 TIMESTAMPTZ（声明"这是 UTC"），再 `'Asia/Shanghai'` 把 TIMESTAMPTZ 降回 naive 但用 BJT 墙钟（再 to_char 输出）。
3. 速查替代：`sl."startTime" + INTERVAL '8 hours'` 也能拿到 BJT 显示，但丢掉 TZ 语义，只在临时排查用。
4. 写 `WHERE startTime BETWEEN '2026-04-28 14:50:00' AND '2026-04-28 15:25:00'` 时，**字面量直接写 UTC 值**（因为列是 naive UTC），别 `'2026-04-28 22:50:00 +08'` 这种带 TZ 的 literal，会触发 PG 隐式转换出意外结果。

> 历史踩坑（2026-04-28）：之前本 skill 在"一键 SQL"里写过 `startTime AT TIME ZONE 'Asia/Shanghai' AS bjt`，导致排查用户报告时把 BJT 23:01 显示成 07:01，差点误判"数据丢了 8 小时"。本节就是为了下次不再踩。
...
SELECT
  to_char(sl.\"startTime\",                                                       'MM-DD HH24:MI UTC') AS utc,
  to_char((sl.\"startTime\" AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Shanghai',     'MM-DD HH24:MI')     AS bjt,
...
# START / END 字面量必须写 UTC（列是 naive UTC，见上文 TZ 陷阱）
# 用户报的是 BJT？记得 -8h 再代入：BJT 23:01 → UTC 15:01
kubectl exec litellm-db-0 -n carher -- psql -U litellm -d litellm -c "
SELECT
  to_char(sl.\"startTime\",                                                       'YYYY-MM-DD HH24:MI:SS UTC') AS utc_start,
  to_char((sl.\"startTime\" AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Shanghai',     'YYYY-MM-DD HH24:MI:SS')     AS bjt_start,
  to_char((sl.\"endTime\"   AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Shanghai',     'HH24:MI:SS')                AS bjt_end,
  EXTRACT(EPOCH FROM (sl.\"endTime\"-sl.\"startTime\"))::int                       AS dur_s,
  sl.completion_tokens AS toks, sl.prompt_tokens, sl.model, sl.spend
FROM \"LiteLLM_SpendLogs\" sl
JOIN \"LiteLLM_VerificationToken\" vt ON sl.api_key = vt.token
WHERE vt.key_alias = '<KEY_ALIAS>'
  AND sl.\"startTime\" BETWEEN '<UTC_START>' AND '<UTC_END>'
ORDER BY sl.\"startTime\" DESC LIMIT 30;"
```
...
- **用户报"在 T1 → T2 调用过 N 次但只看到 1 行"**：先看那 1 行的 `bjt_start ↔ bjt_end` 是否就跨越 T1 → T2。**Claude Code SSE 流式是单次 HTTP 持续 ~30–60s**，前端用户感知"打字的整段时间"≠"调用次数"。SpendLogs 一次 HTTP 写一行，只有 `startTime` 是请求开始时刻，`endTime` 是流结束时刻。

### kubectl logs 时间窗口的硬限制（**别再被它骗一次**）

LiteLLM proxy pod 默认 container log 文件 ~10MB rotation + 高频 INFO，**实测每个 pod `kubectl logs --since=1h` 通常只能拿到最近 10–15 分钟**。
排查 30 分钟以前的请求时：

```bash
# 必看：每个 pod 实际能拿到的日志时间区间
for p in $(kubectl get po -n carher -l app=litellm-proxy -o name); do
  POD=${p#pod/}
  EARLIEST=$(kubectl logs $POD -n carher --since=24h -c litellm 2>&1 \
              | rg -a -o '^.\[..m([0-9]{2}:[0-9]{2}:[0-9]{2}) - ' -r '$1' | head -1)
  LATEST=$(kubectl logs $POD -n carher --since=24h -c litellm 2>&1 \
              | rg -a -o '^.\[..m([0-9]{2}:[0-9]{2}:[0-9]{2}) - ' -r '$1' | tail -1)
  echo "$POD: $EARLIEST ~ $LATEST"
done
```

如果用户报的时间已经超出窗口，**直接放弃 kubectl logs，靠 DB SpendLogs + 预先接好的 OTEL/Datadog**（参考 `docs/litellm-safe-upgrade-canary-plan.md` 的 logging callbacks 段）。**永远不要因为 logs 拿不到就推断"没发生过"**。
...
- ⚠️ **变更必须用 `kubectl set image/env` 或精确 `kubectl patch`，不要用 `kubectl apply -f k8s/litellm-proxy.yaml`**——除非你 15 分钟内亲自核对过仓库 manifest 与生产已完全同步（image、CM 内嵌 `config.yaml`、initContainer 命令都一致）。真相是**这份 manifest 长期漂移**：生产的 image 是历史上 `kubectl set image` 打上去的、CM 是 admin UI / kubectl patch 改的，而仓库版停留在几周前。apply 会同时把 image 回退成陈旧版、把 embedded CM 覆盖回旧值。2026-08-27 事故实例：apply 一步就把 v1.90.2 → v1.85.0 并抹掉手工加的 acct-122~134 池、`gpt-5.6-{terra,sol,luna}` 无前缀改名、`litellm_settings.drop_params: true`，连带 carher-1000 报 `UnsupportedParamsError: tool_choice`。
- 只改 CM 的正确姿势：`kubectl patch cm litellm-config -n carher --type=merge --patch-file=<json>`（patch 内只含 `data.config.yaml` 完整新字符串），保留 metadata/annotations。
- LiteLLM 有 90s initialDelay，要有耐心等滚动更新完成
- 用 `kubectl rollout status` 监控进度
- **LiteLLM proxy 不 hot reload**：CM patch 后必须 `kubectl rollout restart deploy/litellm-proxy -n carher` 才会加载（carher 主程序侧的 5s reloader sidecar 是 carher 特有，litellm 没有）。

## 升级到 v1.90+ 的两处变化（实测 2026-08-27）

1. **`schema.prisma` 路径变了**。v1.85 里在 `/app/litellm/proxy/schema.prisma`，v1.90 迁到 `/app/schema.prisma`（顶层 vendored copy）。仓库 initContainer 命令仍写旧路径 → `prisma-migrate` `Error: Provided --schema at /app/litellm/proxy/schema.prisma doesn't exist` → Init:Error → 新 pod 起不来（旧 pod 仍服务，零中断）。修法：`kubectl patch deploy litellm-proxy -n carher --type=json -p '[{"op":"replace","path":"/spec/template/spec/initContainers/0/command","value":["sh","-c","prisma db push --schema /app/schema.prisma --skip-generate --accept-data-loss"]}]'`
2. **openai provider 严格校验 tool_choice**：v1.90 会 400 `UnsupportedParamsError: openai does not support parameters: ['tool_choice']`，除非 CM 顶层有 `litellm_settings: drop_params: true`。这条常被"CM 恢复/迁移"忽略掉。

## `router_settings.model_group_alias` 陷阱（实测 v1.85）

`model_group_alias: {chatgpt-gpt-5.6-terra: gpt-5.6-terra}` 在 v1.85 里**不生效**：请求进 proxy 时 model 校验发生在 router alias 展开**之前**，报 `Invalid model name passed in model=chatgpt-gpt-5.6-terra`。要做旧名兼容，必须在 `model_list` 里直接追加"别名部署"——`model_name` 用旧名，`litellm_params` 完整复用新组的 upstream（相同 `model` / `api_base` / `api_key` / cost），`model_info.id` 也统一改成旧名，避免 SpendLog 号型混乱。v1.90.2 未验证是否修复，保守做法一律走 model_list 追加。
