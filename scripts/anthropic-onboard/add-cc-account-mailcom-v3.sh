#!/usr/bin/env bash
# add-cc-account-mailcom-v3.sh — single-context patchright 版 (2026-05-25)
#
# 用法:
#   ./scripts/anthropic-onboard/add-cc-account-mailcom-v3.sh acct-N
#
# 与 v1 / v2 区别:
#   - v1: 嵌套 patchright + vanilla → asyncio loop 冲突
#   - v2: 4 段串行 + storage_state 跨 patchright context → D 段白屏
#   - v3: **single patchright context**, 仿 cc-oauth-171mail.py 架构,
#         claude_page 全程 alive (不 close), mail/code page 共享同一 ctx
#
# .creds 字段同 v1/v2: email + mail_pw
...
PY_SCRIPT="$SCRIPT_DIR/cc-oauth-mailcom-v3.py"
...
PY_REMOTE="/tmp/cc-oauth-mailcom-v3.py"

echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  add-cc-account-mailcom-v3.sh  $ACCT"
echo "╚══════════════════════════════════════════════════════════════╝"

echo "==[1/5]== 检查 $CREDS_REMOTE + 上传 v3 脚本"
...
echo "==[3/5]== Docker patchright 跑 single-context OAuth (~3-5min)"
