#!/usr/bin/env bash
# Wrapper: run cc-max-upstream-status.py on 188 (where /Data/anthropic-auth lives).
# All args forwarded.
#
# Usage:
...
#
# Mechanics:
#   1. Copy the .py to 188:/tmp/cc-max-upstream-status.py via jms ssh stdin
...
"$JMS" ssh JSZX-AI-03 "cat > $REMOTE_PATH" < "$PY"

# Step 2: run with args
if [[ $# -gt 0 ]]; then
  ARGS_QUOTED=$(printf '%q ' "$@")
  "$JMS" ssh JSZX-AI-03 "python3 $REMOTE_PATH $ARGS_QUOTED"
else
  "$JMS" ssh JSZX-AI-03 "python3 $REMOTE_PATH"
fi
