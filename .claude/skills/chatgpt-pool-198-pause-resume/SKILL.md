...
- 🔴 **`replicas>0` ≠ 在服务**。2026-09-19 实测 165 个 `chatgpt-acct-*` 里**只 54 个在服务**。
  死号会让"我搞坏了"和"本来就坏"长得一模一样 ⇒ 判据看 `state.terminated.finishedAt`。

## 🔴 改在服务的号:默认不做

2026-09-19 实测:**重启一个正在服务的号可能永久打死它**(acct-237 被我重启后死亡,
**回退配置救不回来**,只能重新登录)。`ready:true` 量不到"重启之后还能不能服务" ——
它只说明这一刻进程在。

⇒ **批量对在服务的号做 pod spec 变更,默认不做。** 真要做:

```bash
scripts/acct-pod-spec-change.sh      # 逐个、带 before 快照、可回滚
```

先只挑**不在服务**的号练一遍(判据同上:`finishedAt` 有值 / arms 为 0),
拿到阳性对照再谈扩大范围。授权"迁移"不等于授权"顺手重启"。
