...
### ⭐ 生产实测:结构重试的性价比很差(2026-07-27)

读了生产 bridge 8 小时日志(861 行):

| 指标 | 值 |
|---|---|
| `HIT` 总数 | 137(r0:114 / r1:12 / r2:8 / r3:3) |
| 结构重试触发 | **83 次** |
| 重试后仍无 tool_call | **76 次** |
| → 救回率上界 | **≤ 7/83 = 8.4%** |

**每次重试都花掉一次上游调用(G4 成本),却只有 ≤8.4% 换来结果。**

但这只是**上界** —— r1 的 12 个 HIT 由结构重试、拒绝重试(11 次)、错误重试
共同贡献,日志无法区分。**所以先不动阈值**,而是加了归因计数器:
`/health` 的 `structural_retry: {fired, rescued, wasted, rescue_rate}`。
...
**已部署并累积到 n=14(2026-07-27)**:

```
/health -> structural_retry: {"fired":14,"rescued":9,"wasted":5,"rescue_rate":0.643}
日志:    HIT zero-88 r1 struct-rescued     ← 旧代码会丢掉这些
```

| 指标 | 改动前 | 改动后 |
|---|---|---|
| 结构重试救回率 | **≤ 8.4%**(上界,83 次触发/76 次失败) | **64.0%**(累计 16/25,归因实测) |
| Wilson 95% CI | — | **[0.445, 0.798]** |
| Fisher vs 改前(7/83) | — | **p < 1e-6** |

**改动前的上界 0.084 落在 CI 之外** → 提升是统计显著的,不是噪声。
这是本轮唯一可以下结论的数字。

⚠️ **计数器是进程内内存,重启即清零**(单副本,无持久化)。
所以 `rescue_rate` 只反映**本次启动以来**的窗口。要长期趋势得在重启前抓取,
或后续接 Prometheus。上面的 n=14 是重启前抓到的。

### ⭐ 追加写 4/10 失败的真凶:弯撇号(2026-07-27,已修)

上一轮我说"剩余原因未查明,ENOENT 只占 6%"。抓失败样本的**完整轨迹**后定位了:

失败样本的模型原话是 **"I don't have access to a shell execution tool"** ——
这是**拒绝**,不是 ENOENT。而 `_looks_like_refusal()` 漏判了它。

单变量隔离(两句话除撇号外完全相同):

| 文本 | 判定 |
|---|---|
| `I don't have access to a shell...`(直撇 U+0027) | **REFUSAL** ✅ |
| `I don’t have access to a shell...`(弯撇 U+2019) | **NOT-REF** ❌ |

检测器里 ~25 个 pattern 全写直撇(`can't`/`don't`),而模型输出的是
**typographic 撇号 U+2019**。漏判 → 该 pod 不被惩罚 → 拒绝文本被当成最终答案返回。

修:在 `_looks_like_refusal()` **入口**归一化 `’‘""…`,而不是改 25 个 pattern
(更小,且新增 pattern 时不会忘)。
...
| | 修复前 | 修复后 |
|---|---|---|
| 追加写成功率 | 6/10 | **8/8**(本轮),累计 **19/20** |
| Fisher p(累计 19/20 vs 6/10) | — | **0.031** ✅ 显著 |
| Wilson 95% CI | — | [0.764, 0.991] |
| 拒绝语料 | 30/30 | 30/30(无退化) |
| KEEP 误判 | 0/26 | 0/26 |

`testkit/test_refusal_corpus.py` 9/9 钉住,已验证破坏归一化会 FAIL(2 条)。

### 顺带纠正一个我自己的测量错误

我手写打分时把 `STALL` 当成"期望 False"来判,得出"4 条失败"的**假退化**,
差点去改一个没坏的东西。`corpus.score()` 的口径是 **STALL 期望被捕获(True)**。
新测试改为直接调用 `corpus.score()`,不再自己定义口径。
...
### ⭐ 用户实测报告的最严重 bug:多问题会话串题(2026-07-28,已修)

用户反馈"5.6 Luna 把我上次的问题回答了,没回答本次的"。查
`~/.codex/archived_sessions/rollout-2026-07-28T09-12-52-*.jsonl` 拿到决定性证据:
...
**工具调用是对的**,说明模型理解了新问题。错在 `_original_goal()`:
它**正向扫描取第一个用户问题**,于是 nudge 告诉模型
`ORIGINAL REQUEST: 快速排序`。在"判断请求是否已回答"那一轮,
模型回答了**我们塞给它的目标**,而不是用户真正问的。

**这是我们覆盖了用户意图** —— 比模型能力问题严重得多,因为它把一个
正确的执行结果丢掉了。

修:改为**反向扫描取最新**的真实用户轮,并跳过
`<environment_context>` / AGENTS.md 前言 / `[result of ...]` /
我们自己注入的 `[ERROR] ...` 轮(否则会把自己的 nudge 当成用户目标)。

验证:
- 离线 `testkit/test_original_goal.py` **14/14**(已验证改回正向扫描会 FAIL 2 条)
- 真实路径 `testkit/cx_multi_question.py`:先问快排再问磁盘 →
  答案是 **926GiB 磁盘信息,不含快排** ✅
- 全功能回归仍 **8/8**,无副作用
