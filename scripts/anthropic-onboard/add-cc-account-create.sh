...
PY_SCRIPT="$SCRIPT_DIR/cc-oauth-create-account.py"
...
RUNNER_LOG="/tmp/cc-oauth-$ACCT-runner.log"
PY_REMOTE="/tmp/cc-oauth-create-account.py"
...
CALLBACK=$(ssh "$SSH_188" "
    ...
    docker run --rm \
      ...
      bash -c 'Xvfb :99 -screen 0 1280x800x24 >/dev/null 2>&1 & sleep 1 && \
               ...
               python3 /work/script.py' 2>&1 | tee $RUNNER_LOG | grep '^✅ CALLBACK_CODE=' | tail -1 | sed 's/.*CALLBACK_CODE=//'
    rm -f $DOCKER_ENV_REMOTE
")
if [[ -z "$CALLBACK" ]]; then
    echo "❌ OAuth 失败. 截图: ssh $SSH_188 ls $SS_DIR"
    echo "    runner log: ssh $SSH_188 tail -40 $RUNNER_LOG"
    exit 2
fi
