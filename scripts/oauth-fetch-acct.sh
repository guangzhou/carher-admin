...
echo "==[1/6]== 复制 device-code 脚本到 198"
cat "$SCRIPT_SRC" | jms scp - AIYJY-litellm:$SCRIPT_198

echo "==[2/6]== 198 上后台起 poll（${POLL_MIN}min 窗口）"
# setsid + disown 防止 jms ssh exit 时把进程一起杀
jms ssh AIYJY-litellm "
  pkill -f 'chatgpt-device-manual.py' 2>/dev/null || true
  rm -f $LOG_198 /tmp/auth-acct-${ACCT}.json
  setsid bash -c 'AUTH_JSON_OUTPUT=/tmp/auth-acct-${ACCT}.json POLL_MINUTES=${POLL_MIN} python3 ${SCRIPT_198} > ${LOG_198} 2>&1' < /dev/null > /dev/null 2>&1 &
  disown
  sleep 3
  PIDS=\$(pgrep -f chatgpt-device-manual.py | tr '\n' ' ')
  echo \"  poll PIDs: \$PIDS\"
" || true
