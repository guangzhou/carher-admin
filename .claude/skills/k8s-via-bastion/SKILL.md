...
|----------|---------|---------|------|------|
| **`laoyang`** | 43.98.160.216 | 172.16.0.228 | 阿里云 ECS（工具节点） | 自带 docker / git / python3 / cloudflared / ansible，但**没有 kubectl / nerdctl**；本地 ext4 `/Data`（不是 NAS）。**⚠ KoKo exec channel 坏死，`jms ssh` 必须 `--tty`，`jms proxy` 不能走此节点** |
| **`k8s-work-227`** | 47.84.112.136 | 172.16.0.227 | K8s worker + 主构建机 | `/root/carher` + `/root/carher-admin` + `nerdctl` + NAS `/Data` |
| **`k8s-work-226`** | 47.84.112.136 | 172.16.0.226 | K8s worker + 备构建机 + **默认 kubectl 隧道出口** | `/root/carher-admin` + `nerdctl` + NAS `/Data`（无 carher 主仓） |
...
> **kubectl 隧道默认走 `k8s-work-226`**（2026-07-12 起由 laoyang 改为 226，
> 因为 laoyang 的 KoKo exec channel 坏死导致 `jms proxy laoyang` TCP relay 卡死）。
> 技术上 `jms proxy k8s-work-22{7,9} 16443 172.16.1.163 6443` 也能用。
> **laoyang 上不要直接跑 kubectl**——它没装；如要在该机上执行 kubectl，
> 改用本地 Mac + `jms proxy k8s-work-226` 隧道（场景 1）。
...
| `scripts/jms ssh <asset> [cmd...]` | 远程命令 / 交互式 shell（**laoyang 必须加 `--tty`**） |
...
```bash
nohup scripts/jms proxy k8s-work-226 16443 172.16.1.163 6443 \
  > /tmp/jms-proxy.log 2>&1 &

sleep 2 && kubectl get nodes   # 验证
```

> ⚠️ **不要用 `laoyang` 做 proxy 出口**——它的 KoKo exec channel 坏死，
> TCP relay（`ssh laoyang "nc host port"`）会卡死。已于 2026-07-12 改为 `k8s-work-226`。
>
> 出口选 `k8s-work-226` 是当前约定默认值。任何 worker（`k8s-work-22{7,9}`）
> 也能 nc 到 apiserver，用它们替换 asset 名一样能跑。
...
```bash
pkill -f 'jms.*proxy k8s-work-226'
```

**检查是否已经在跑**：

```bash
pgrep -af 'jms.*proxy k8s-work-226' && echo "running" || echo "not running"
```

**多个隧道并存**（不同本地端口即可）：

```bash
nohup scripts/jms proxy k8s-work-226 16443 172.16.1.163 6443 > /tmp/p1.log 2>&1 &
...
> **⚠️ laoyang 非交互命令必须加 `--tty`**。KoKo 对该资产的 exec channel
> 永久卡住（2026-07-12 实证），普通 `jms ssh laoyang "cmd"` 会挂死。
> `--tty` 通过强制分配 PTY + stdin 管道 + sentinel 检测来绕过。
> 其他资产（226/227 等）**不需要** `--tty`。

```bash
scripts/jms ssh laoyang                                  # 交互式 shell（不需要 --tty）
scripts/jms ssh --tty laoyang 'uname -a && uptime'       # 一次性命令（⚠ 必须 --tty）
scripts/jms ssh --tty laoyang 'docker ps'                # docker 仍然可用
scripts/jms ssh --tty --timeout 60 laoyang 'docker logs --since 30m carher-1'  # 长命令加 --timeout
```

> ⚠️ laoyang 上**没有 kubectl**——不要在它上面跑 `kubectl get pods`。
> 任何 kubectl 需求都在 Mac 本地跑，前提是场景 1 的 `jms proxy` 隧道起着。
>
> ⚠️ `--tty` 模式不支持 heredoc（`"bash -s" <<'REMOTE'`），改用 `&&` 串接。

文件传输同样支持（scp 不受 TTY 影响）：
...
```bash
nohup scripts/jms proxy k8s-work-226 16443 172.16.1.163 6443 > /tmp/jms-proxy.log 2>&1 &

scripts/jms ssh --tty laoyang "docker ps"
scripts/jms ssh --tty laoyang "systemctl status cloudflared"
