...
- 地板值是量出来的，不是挑的。阴性对照（stable 劈半对自己，2026-09-16..18，2123 个
  5 分钟 class-窗口，真值必须为 0）：地板 **100 判红 3.35%**，地板 **200 判红 0.00%**，
  而注入 +2pp 的阳性对照检出率 100.0%。这里地板越高**严格更优**，不是灵敏度取舍。
- **5xx 腿必须留在 5 分钟窗。加宽只会更糟**：同样地板 200，30 分钟窗判红 **7.98%**——
  长窗口跨越了真实的流量结构变化，样本不再同质。
- 样本不够时报 `FIVE_XX_SAMPLE_BELOW_FLOOR` 诚实熄灯，而不是拿不够的请求数算出一个
  比率装作武装。覆盖率 26.8%（有流量就武装，没流量就熄灯并说出来）。
