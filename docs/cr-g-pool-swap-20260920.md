...
## 九、第四批：205 入池，165/166 被门禁挡下（09-21 02:1x ~ 02:3x）

### 9.1 门禁第一次真的拦住了东西 —— 而且不是因为腿坏

205/165/166 三条同批抓成（都是 A 类换表里密码抓的），**灌装、in-pod、直打 lane 三关全过**：

```
✅ lane 205  http=200 暗号=True      ✅ lane 165  http=200 暗号=True
✅ lane 166  http=200 暗号=True      ✅ lane 193  http=200 暗号=True   ← 阳性对照
```

**但目录门禁 1/3**。165/166 回的是**免费档菜单**：

| 腿 | slug 数 | 独有 | 缺 ref 193 的 |
|---|---|---|---|
| 205 | 21 | — | 0 ✅ |
| 165 | 13 | `auto`, `gpt-5-6-t-mini-mini` | 10 个（thinking/pro/instant 全族 + o3-pro + gpt-6-*） |
| 166 | 13 | 同上 | 同上 |

两次读完全一致 ⇒ **不是刚登录的瞬态，是账号档位**。`auto` 这个 slug 只在免费档出现，
付费档反而没有 —— 所以"独有 2 个"不是加分项，是降级的指纹。

**这一条要单独记住：腿活 ≠ 该号买了订阅。** 直打 lane 200 + 命中暗号只证明
「seed 有效、会话能用」，证不到「这个号手里有 pro/thinking」。反过来，七节里 195 的教训是
「实打 403 也不证明腿死」。⇒ **直打和目录门禁量的是两件不同的事，两道都要过，谁也替不了谁。**

### 9.2 处置

- **205 入池**：注册 15/15 OK，`rollout restart` 后独占直名 `cr-g-5.6-mini-205` **2/2 全绿**
  （2.8s / 2.7s）。**池腿 26 → 27。**
- **165/166 不入池**，deploy `scale --replicas=0`。
  - 动了啥：两条 Deployment 副本数 1 → 0。模型行**一行都没建**（门禁在注册之前挡下的）。
  - 备份在哪：`198:/tmp/bk-zero-cursor-bpi-165-20260921.yaml`（5653 B）、
    `198:/tmp/bk-zero-cursor-bpi-166-20260921.yaml`（5652 B）。
    seed 保留在 `225:/Data/zerokey-sessions/zero-16{5,6}/users.json`。
  - 怎么回滚：账号买上订阅后 `kubectl -n litellm-product scale deploy/zero-cursor-bpi-16X
    --replicas=1`，重跑目录门禁确认变成 21 slug，再 `crg_pool_register.py --lanes 16X
    --with-direct --apply`。**不用重抓号** —— seed 是好的，缺的只是订阅。

### 9.3 回归

`--all --keys 10`：**14/14 命中、失败 0 发**，落点 177/182/185/188/193/195/198/200/202/204/207/208。

**诚实边界**：这一轮 15 条腿（含 205、196、197、199）没被抽到。27 条腿要靠 14 发打全
是不可能的 —— 亲和钉的是 key，不是请求。每条新腿的个体证据是「直打 lane 绿 + 独占直名绿」，
池名层面的覆盖只能随时间累积，**别把 `--keys N` 那一轮的绿说成"27 条腿都验过了"**。

## 十、第五批：167 入池（27→28），168 同样被门禁挡下（09-21 02:4x ~ 03:0x）

| 号 | 抓号 | 灌装+in-pod | 直打 lane | 目录 | 结果 |
|---|---|---|---|---|---|
| 167 | ✅ 20576 B | ✅ startedAt 17:52:02Z | ✅ 200+暗号 | ✅ 21 slug，差集 0 | **入池**，注册 15/15，独占直名 2/2 绿（3.8s/2.6s） |
| 168 | ✅ 19311 B | ✅ startedAt 17:54:30Z | ✅ 200+暗号 | ❌ 13 slug 免费档 | **scale 0**，一行模型行没建 |

168 的 slug 列表与 165/166 **逐字相同**（`auto` + `gpt-5-6-t-mini-mini`，缺 thinking/pro/instant
全族 + `o3-pro` + `gpt-6-*`）⇒ 同一个成因：号没订阅。
备份 `198:/tmp/bk-zero-cursor-bpi-168-20260921.yaml`（5653 B），seed 留在 225，回滚同 9.2。

**A 类重抓到目前 9/9 抓号全成（196/197/198/199/205/165/166/167/168），零失败。**
但**入池只有 6 条**（196/197/198/199/205/167）—— 另外三条是**腿活但号没订阅**，
这是两个独立的失败面，不要合并计数：

```
抓号成功率   9/9    ← 换密码来源这根杠杆的效果
可入池率     6/9    ← 账号本身有没有买订阅，与抓号无关
```

回归 `--all --keys 6`：14/14 命中、失败 0，落点含新腿 167/197/199。

**池腿 27 → 28。** `POOL_LANES` 同步（写成字面量列表，不用 `.append()`——
那样 `ast.literal_eval` 读不到，自检脚本会瞎）。

## 十一、A 类重抓收尾与 B/C 类的阴性结论（09-21 03:0x）

### 11.1 A 类 10/10 抓号全成，但只有 6 条能入池

| 号 | 抓号 | 直打 lane | 目录 slug | 结果 |
|---|---|---|---|---|
| 196 | ✅ 20159 B | ✅ | 21 | **入池** |
| 197 | ✅ 19123 B | ✅ | 21 | **入池** |
| 198 | ✅ 20224 B | ✅ | 21 | **入池** |
...
| 205 | ✅ 20204 B | ✅ | 21 | **入池** |
| 167 | ✅ 20576 B | ✅ | 21 | **入池** |
| 165 | ✅ 20210 B | ✅ | **13** | scale 0（号没订阅）|
| 166 | ✅ 20183 B | ✅ | **13** | scale 0（号没订阅）|
| 168 | ✅ 19311 B | ✅ | **13** | scale 0（号没订阅）|
| 169 | ✅ 20004 B | ✅ | **13** | scale 0（号没订阅）|

（172 在跑，是最后一个。）**四条免费档的 slug 列表逐字相同**，直打 lane 全是
200 + 命中暗号 ⇒ 腿都活着，缺的只是订阅。四条的 YAML 备份都在
`198:/tmp/bk-zero-cursor-bpi-16{5,6,8,9}-20260921.yaml`，seed 留在 225，
回滚 = 账号买订阅后 `scale 1` → 重跑门禁 → 注册，**不用重抓号**。

**两个失败面必须分开计数**：换密码来源治的是「抓不到号」，治不了「号没买订阅」。
把 6/10 说成"重抓成功率 60%"是错的；抓号是 10/10。

acct-169 中途出现 `mail.com: waiting inbox... [44/45]` + `skeleton stall — reload`
差点判成 B′，**最后一轮自己收到码了** ⇒ 收件箱等待到 [44/45] 还不算完，别提前判死。

### 11.2 B 类（183/184/189/203）、C 类（192/194）：换密码这根杠杆不适用

**逐号比值，六个号盘上 `mail_pw` 与表里完全一致**：

| 号 | 盘 sha12 | 表 sha12 | 长度 | 判定 |
|---|---|---|---|---|
| 183 | a6b3dc763b5b | a6b3dc763b5b | 13 | **相同** |
| 184 | 33e458827a85 | 33e458827a85 | 12 | **相同** |
| 189 | 0daf80cab57a | 0daf80cab57a | 13 | **相同** |
| 203 | 2255ca33f391 | 2255ca33f391 | 14 | **相同** |
| 192 | 0cfda955127a | 0cfda955127a | 12 | **相同** |
| 194 | ad587c6a7e0f | ad587c6a7e0f | 13 | **相同** |

⇒ 拿 `lane_recapture_sheet_pw.sh` 重跑这六个号是**空操作**（脚本会自己打「盘 == 表」），
它们的失败与密码漂移无关。这是**阴性结论，不是"还没查"**：A 类那 10 个全是「盘 ≠ 表」
且长度 17（轮换后形状），这六个全是「盘 == 表」且长度 12–14（轮换前形状）
—— 两组在这一维上**完全不重叠**，所以密码这根杠杆解释得了 A 类，解释不了 B/C 类。

下一步该查的是别的轴（B 类 = 有码但 `logged_in=False`、C 类 = 导航超时），
不该再在密码上打转。

---

## 十二、acct-172 收尾：A 类最后一个号，池 28 → 29 腿（09-21 03:0x）
...
| ① 字节数 | 21570（源 = 盘，一致） |
| ② users key | `['acct172']` |
| ③ parsedFetch | headers=36、cookie=8047B、sentinel=True |
| ④ in-pod | 容器 `startedAt=2026-09-20T18:30:15Z` **晚于** seed mtime=1789929006，key=acct172 |

④ 能量得准是因为复跑 `lane_seed_install.sh 172` 打的是「**内容与现有 seed 一致，未覆盖**
（保留原 mtime）」—— 一旦它覆盖了就把自己的量具毁了。备份落在
`188:/Data/backups/zerokey-seed-172-20260921-023030-pre-install.json`（21570 bytes）。

### 12.2 直打三极同轮（193 正 / 172 待判 / 82 负）

```
✅ lane 193  http=200 暗号=True 收割=14B
✅ lane 172  http=200 暗号=True 收割=14B
❌ lane 82   http=500 暗号=False 收割=0B <session_expired>
```

**负对照必须在同一轮**：只有正对照的"全绿"证不了这把尺子今天还能变红。82 当场红，
说明这轮的判死能力是活的。

⚠️ **尺子坑（我先踩了一次）**：`/tmp/lanehealth.py` 第 2 行是
`LANES = sys.argv[1].split(",")` —— 吃的是**逗号串**。我第一次写成位置参数
`python3 - 172 193`，它静默只打了 172；第二次 `193 172 82` 又只打了 193。
两次都印出一行"✅"，形状和"两腿都绿"**一模一样**，但那一轮**根本没有对照**。
正确形状是 `python3 - 193,172,82`。

### 12.3 目录门禁

lane 172 与参照腿 193 **都是 21 slug、双向差集 0**（含 thinking/pro/instant），
`GATE PASS 1/1`。`plan=HTTP 500 {"error":"Invalid version: v4"}` 两条腿同样报，
是既存故障不是 172 的问题。

### 12.4 注册与回归

- `crg_pool_register.py --lanes 172 --with-direct --apply`（proxy pod 内）：**15/15 OK**
- `rollout restart deploy/litellm-proxy` → rolled out
- 现网复核（`/model/info` 在 pod 内聚合）：**池名数 14，每个池名 29 腿**，
  腿集合 = POOL_LANES，无 82/84/135、无 165/166/168/169
- 独占直名 `cr-g-5.6-mini-172` 实打：**命中暗号，收割 16B** ← 唯一能证明"这条腿在服务"的尺子
- 池级回归 `--all --keys 8`：**14/14 绿、失败 0 发**，落点里 172 自然吃到 2 发（key1、key3）

### 12.5 A 类重抓总账（收尾）

| | 号 | 数 |
|---|---|---|
| 抓号成功 | 196 197 198 199 205 165 166 167 168 169 **172** | **11/11** |
| 入池 | 196 197 198 199 205 167 **172** | **7** |
| 门禁挡下（免费档，scale 0 留 seed） | 165 166 168 169 | **4** |

**两个失败面仍然分开记**：换密码这根杠杆的命中率是 **11/11 = 100%**；
没进池的 4 个是**账号没买订阅**，与抓号无关。合并成"64% 成功"会把一个
完全有效的修复误判成半成品。
