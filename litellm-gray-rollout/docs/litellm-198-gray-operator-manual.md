...
- 样本不够时报 `FIVE_XX_SAMPLE_BELOW_FLOOR` 诚实熄灯，而不是拿不够的请求数算出一个
  比率装作武装。
- 🔴**熄灯不等于修好了。按当前 15% 分流，5xx 腿在 5 分钟窗的武装率是 0.0%**
  （逐 class，两侧都要够 200）。这个修复让门禁不再撒谎，但没让它变成活的止损。
  实测武装率（阴性对照同一份日志，真实推理 class）：

  | 分流 | 逐 class 5 分钟 | 逐 class 15 分钟 |
  |------|----------------|-----------------|
  | 15%  | **0.0%**       | 1.5%            |
  | 50%  | 3.0%           | 19.6%           |
  | 100% | 13.5%          | 30.0%           |

  「把 5 个 class 合成一条总线」试过了，**不解决问题**：武装率虽然能到 5 分钟 38.3%
  （50% 分流），但阴性对照假阳 **8.21%**，和逐 class 的 10.94% 一个量级，
  而且把哪个 class 坏了这个信息丢掉了。
  ⇒ 结论：**5xx 腿只有在分流本身上去之后才真正武装**，靠改地板/改窗口/改聚合口径
  都换不来。接 dispatcher 前这一条必须写进执行单，不能让人误以为止损已经在岗。
