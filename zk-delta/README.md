...
**两条上限都得留着。** 哪一条先拦住随会话大小翻转：`audit.sh` 第 [7] 组是**当场按均值算**的
（`400 × 当前均值`），均值 4.6MB（长会话）时字节上限先到，均值 0.8MB（刚重启、都是短会话）
时条数上限先到。上表那个会 OOM 的算例取的是长会话——**坏情况才是定上限的依据**，
所以别把某一次巡检打印的那句"真正在拦的是 X"当成结论背下来。
（每人 8 条会话是**假设**不是实测——只有我自己一台机器的行为数据。）
