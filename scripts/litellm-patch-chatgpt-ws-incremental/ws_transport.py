...
# 默认 14MiB 留余量（实测用 ASCII，多字节内容序列化尺寸有方差）。超限帧本地闸门直接走
# HTTP（分块引导已实测退役：prev 链绕过服务端 HTTP 截断 → context_length_exceeded 必炸）。
    ...
    *,
    ...
    try:
        ...
        if frame_bytes > _WS_MAX_FRAME_B:
            # 超限 delta（罕见）：同 full 路闸门，走 HTTP（见 _full_ws_turn 内注释）。
            _log(
                f"ws_incr_fallback reason=frame_oversize pck={_pck8(pck)} src={sess.pck_src} "
                f"frame_kb={frame_bytes // 1024} items={len(delta_items)} limit_kb={_WS_MAX_FRAME_B // 1024}"
            )
            await sess.destroy()
            return None
...
async def _full_ws_turn(*, pck, pck_src, full_reason, ws_url, ws_headers, data, cur_hashes, props_hash,
    ...
    try:
        ...
        frame_bytes = len(json.dumps(frame).encode("utf-8", "replace"))
        if frame_bytes > _WS_MAX_FRAME_B:
            # 超上游 16MiB 单消息上限（1009，二分实测）→ 本地闸门直接走 HTTP，不白费连接。
            # ⚠ 分块引导（prewarm 链）已实测并退役：prev 链在服务端**绕过 HTTP 路径的自动
            # 截断**——9MB 请求 HTTP 只计 ~10K tokens（截断救活），链式却累积 209 万 tokens
            # 后 context_length_exceeded 必炸 + 消耗 200× 配额。字节超限 ⇒ token 必超限
            # ⇒ 该类会话的正确归宿就是 HTTP（服务端截断）。修复其流量的正道是客户端压缩，
            # 不在本层。
            _log(
                f"ws_incr_fallback reason=frame_oversize pck={_pck8(pck)} src={pck_src} "
                f"frame_kb={frame_bytes // 1024} items={len(data['input'])} limit_kb={_WS_MAX_FRAME_B // 1024}"
            )
            await sess.destroy()
            return None
