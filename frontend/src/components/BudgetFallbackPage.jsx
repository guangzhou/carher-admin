...
function DetailPanel({ row, events, busy, onClose, onAction }) {
  ...
  return <aside className="budget-detail"><div className="budget-detail-title"><div><p>KEY POLICY</p><h3>{row.key_alias}</h3></div><button type="button" onClick={onClose} aria-label="关闭详情">×</button></div>
    ...
    <div className="budget-actions"><button disabled={busy || !row.enabled} onClick={() => onAction("fallback")}>立即切到 5.3</button><button disabled={busy || !row.enabled} onClick={() => onAction("restore")}>恢复主模型</button>
      <button disabled={busy || !row.enabled} onClick={() => onAction("recapture")}>重新采集配置</button><button disabled={busy || !row.enabled} onClick={() => onAction("pause")}>{row.automation_paused ? "恢复自动控制" : "暂停自动控制"}</button></div>
