...
## 4.6 全量上线:52/52(2026-07-28)

```
第一批(手动 5 个)      101-105    OK 5/5,每个 24 actions
第二批(后台全量)      其余        OK 47,FAIL 0,SKIP 5
补跑 SKIP 的 5 个        87-91      OK 5/5
独立复查(全量重跑)                OK 50,SKIP 2  -> 补跑 96/97  OK 2/2
最终                                **52/52,全部 actions=24,FAIL 0**
```

### SKIP 的真因:pod 名在批量途中失效

第一次 SKIP 的是**连号 87-91**,第二次是 96/97 —— 连号不像随机抖动。
查到 pod 年龄只有 13~22 分钟:**这些 pod 在批量运行期间重启了**,
工具开头一次性解析的 pod 名变成了死名字,`kubectl exec` 自然读不到 `users.json`。

不是 pod 坏、也不是账号坏 —— 重新解析 pod 名后补跑全部一次通过。
**教训:长批量不能在开头把 pod 名缓存一次用到底。**

### 抽查:批量装的账号真能用

在 **acct110**(批量装的,不是最小闭环用的 acct100)做真实会话调用:

| 尝试 | 结果 |
|---|---|
| #1 | `api_tool.call_tool` 触发,但无数据 |
| #2 | `api_tool.call_tool` 触发,**返回真实飞书数据** |

→ 与 §5 记录的"偶发 blocked by safety checks"同一现象:**间歇性,不是配置问题**。
重试即可。这也再次说明**单次失败不能判死**。
