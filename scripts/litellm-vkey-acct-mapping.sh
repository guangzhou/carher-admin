...
case "$WINDOW" in
  ...
  *) echo "usage: $0 [Nh|Nm|Nd] [--raw]" >&2; exit 2 ;;
esac
...
SELECT
  ...
  COUNT(*) AS calls,
  COUNT(DISTINCT REGEXP_REPLACE(model_id, 'chatgpt-acct-([0-9]+)/.*', 'acct-\1')) AS distinct_accts,
  STRING_AGG(DISTINCT REGEXP_REPLACE(model_id, 'chatgpt-acct-([0-9]+)/.*', 'acct-\1'), ',') AS accts_used,
  -- mode: 最频繁的 acct (取第一个 mode)
  MODE() WITHIN GROUP (ORDER BY REGEXP_REPLACE(model_id, 'chatgpt-acct-([0-9]+)/.*', 'acct-\1')) AS primary_acct
FROM "LiteLLM_SpendLogs"
WHERE "startTime" > NOW() - INTERVAL '${INTERVAL}'
  AND model_id LIKE 'chatgpt-acct-%/%'
  AND metadata->>'user_api_key_alias' LIKE 'carher-%'
GROUP BY 1;
...
SELECT
  ...
  COUNT(*) AS calls
FROM "LiteLLM_SpendLogs"
WHERE "startTime" > NOW() - INTERVAL 'INTERVAL_PLACEHOLDER'
  AND model_id LIKE 'chatgpt-acct-%/%'
  AND metadata->>'user_api_key_alias' LIKE 'carher-%'
GROUP BY 1, 2
ORDER BY 1, 3 DESC;
SQL
sed -i.bak "s|INTERVAL_PLACEHOLDER|${INTERVAL}|" /tmp/vkey-acct.sql
rm -f /tmp/vkey-acct.sql.bak
fi

kubectl cp /tmp/vkey-acct.sql carher/litellm-db-0:/tmp/vkey-acct.sql 2>/dev/null
kubectl exec -n carher litellm-db-0 -- psql -U "$USER" -d "$DB" -P pager=off -f /tmp/vkey-acct.sql
kubectl exec -n carher litellm-db-0 -- rm -f /tmp/vkey-acct.sql >/dev/null 2>&1 || true
